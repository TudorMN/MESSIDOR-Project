...
def train_module(load_type, adapt, eq_type, opt):
    ...
    if (load_type == 0):
        x, y, input_shape = img_functions.load_data()
        name = "DEFAULT + "
    if (load_type == 1):
        x, y, input_shape = img_functions.load_data_eq()
        name = "EQUALIZED + "
        
    if (adapt == 1):
        x = img_functions.adaptive_equalize_images(x)
        name = name + "ADAPTIVE + "

    class_weights = import_lib.class_weight.compute_class_weight('balanced', import_lib.np.unique(y), y)
    y = import_lib.keras.utils.to_categorical(y)
    x_train, x_test, y_train, y_test = import_lib.train_test_split(x, y, test_size=0.3, random_state=42)

    #eq_type: 0 HSV, 1 RGB, 2 YUV, -1 default
    if (eq_type == -1):
        name = name + "NO-EQUALIZATION"
    if (eq_type == 0):
        x_train = img_functions.equalize_images(x_train, 0)
        x_test = img_functions.equalize_images(x_test, 0)
        name = name + "HSV"
    if (eq_type == 1):
        x_train = img_functions.equalize_images(x_train, 1)
        x_test = img_functions.equalize_images(x_test, 1)
        name = name + "RGB"
    if (eq_type == 2):
        x_train = img_functions.equalize_images(x_train, 2)
        x_test = img_functions.equalize_images(x_test, 2)
        name = name + "YUV"
...
def experimentation_module():
    # 2 classes
    sgd = import_lib.SGD(lr=0.01, nesterov=True)
    ...
    rms = import_lib.keras.optimizers.RMSprop(lr=0.01)
    ...
    adadelta = import_lib.keras.optimizers.Adadelta(lr=0.01)
    ...
    adam = import_lib.keras.optimizers.Adam(lr=0.01)
